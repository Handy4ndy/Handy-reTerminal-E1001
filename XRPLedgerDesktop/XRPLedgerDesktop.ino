/**
 * XRP Ledger Desktop - Minimal Version
 * Dedicated XRP Ledger stream monitor with WiFi portal
 * 
 * Features:
 * - WiFi connection via captive portal
 * - Real-time XRP Ledger stream display
...
void drawBackground() {
    ...
    do {
        ...
        display.drawBitmap(0, 0, xrpl800x480, BITMAP_WIDTH, BITMAP_HEIGHT, GxEPD_WHITE);
...
void updateLedgerDisplay() {
    ...
    do {
        ...
        printCenteredLeft("Learn more at xrpl.org", 450);
    } while (display.nextPage());
...
void updateServerDisplay() {
    ...
    do {
        ...
        printCenteredRight("XRP Ledger Stream", 420);
    } while (display.nextPage());
...
void webSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
    switch (type) {
        ...
        case WStype_CONNECTED:
            usbSerial.println("WebSocket Connected to XRP Ledger");
...
void setupWiFi() {
    ...
    do {
        ...
        display.setFont(&FreeMonoBold18pt7b);
        display.setTextColor(GxEPD_BLACK);
        printCenteredLeft("XRP Ledger", 200);
        
        display.setFont(&FreeMonoBold12pt7b);
        printCenteredLeft("A decentralized blockchain", 250);
        printCenteredLeft("built for fast, low-cost", 280);
        printCenteredLeft("payments & tokenization.", 310);
        printCenteredLeft("Secure, sustainable, and", 360);
        printCenteredLeft("designed for global use.", 390);
        printCenteredLeft("Learn more at xrpl.org", 440);
    } while (display.nextPage());
    ...
    do {
        ...
        printCenteredRight("XRPLedger-Setup", 240);
        printCenteredRight("192.168.4.1", 300);
    } while (display.nextPage());
    
    if (!wifiManager.autoConnect("XRPLedger-Setup")) {
...
void setup() {
    ...
    preferences.begin("xrpl", true);  // Read-only mode
    ...
    usbSerial.println("Connecting to XRP Ledger...");
